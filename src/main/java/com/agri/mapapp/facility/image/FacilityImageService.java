...
public class FacilityImageService {
    private final FacilityImageRepository repo;
    ...
    public record FacilityImageDto(Long id, String url, String originalName, String contentType, Long sizeBytes, Instant createdAt) {}

    @Transactional(readOnly = true)
    public List<FacilityImageDto> list(Long facilityId) {
        return repo.findByFacility_IdOrderByCreatedAtDesc(facilityId).stream()
                .map(i -> new FacilityImageDto(
                        i.getId(),
    ...
    @Transactional
    public FacilityImageDto upload(Long facilityId, MultipartFile file, Long userId) throws IOException {
        ...
        Path facilityDir = baseDir().resolve("facility").resolve(String.valueOf(facilityId)).normalize();
        Files.createDirectories(facilityDir);
        Path dest = facilityDir.resolve(generated).normalize();
        Path base = baseDir();
        if (!dest.toAbsolutePath().startsWith(base)) {
            throw new SecurityException("Invalid path");
        }
        Path tmp = Files.createTempFile(facilityDir, ".tmp-", type.getExt());
        ...
        FacilityImage saved = repo.save(FacilityImage.builder()
                .facility(facility)
                .filename(rel)
        ...
        return new FacilityImageDto(saved.getId(), "/uploads/" + saved.getFilename(), saved.getOriginalName(), saved.getContentType(), saved.getSizeBytes(), saved.getCreatedAt());
    }

    @Transactional
    public void delete(Long imageId) {
        FacilityImage img = repo.findById(imageId).orElseThrow(() -> new IllegalArgumentException("Image not found"));
        Path base = baseDir();
